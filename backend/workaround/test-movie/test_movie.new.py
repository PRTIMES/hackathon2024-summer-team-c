...
def generate_movie(image_path: str, background_path: str, subtitles: list):
    ...
    # 背景画像のアスペクト比を維持しつつ、動画サイズに合わせる
    background_clip = background_clip.resize(height=movie_size[1])  # 高さを基準にリサイズ
    if background_clip.w > movie_size[0]:  # 幅が超えている場合は幅も調整
        background_clip = background_clip.resize(width=movie_size[0])
    ...
    # 背景クリップの持続時間を設定
    background_clip = background_clip.set_duration(image_clip.duration)
    ...
    # 画像クリップとサブタイトルクリップを合成
    final_clip = CompositeVideoClip([ background_clip, image_clip] + subtitle_clips)
