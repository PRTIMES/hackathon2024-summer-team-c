...
def generate_movie(image_path: str, subtitles: list):
    ...
    # 色反転画像クリップを作成
    inverted_image_clip = image_clip.fl_image(invert_colors)

    # 色反転した画像クリップを背景に設定
    inverted_background = inverted_image_clip.on_color(size=movie_size, pos="center")
    ...
    # 動画の長さを15秒に設定
    video_duration = 15
    inverted_background = inverted_background.set_duration(video_duration)
    image_clip = image_clip.set_duration(video_duration)
    
    # BGMを読み込んで音量を調整
    bgm = AudioFileClip("maou_bgm_piano25.mp3").volumex(0.25)
    bgm = bgm.subclip(0, video_duration)  # BGMの長さを15秒にカット
    inverted_background = inverted_background.set_audio(bgm)
    ...
    # 色反転背景、元の画像クリップ、サブタイトルクリップを合成
    final_clip = CompositeVideoClip([inverted_background, image_clip] + subtitle_clips)
