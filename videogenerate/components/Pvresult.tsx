...
const Pv = async (companyId, releaseId) => {
...
const Pvresult = ({ data, setData }) => {
  ...
  const handleClick = async () => {
    try {
      const result = await Pv(companyId, releaseId);
      setData(result); 
      setError(null); // エラーをリセット
    } catch (err) {
      setError(err.message);
      setData(null); // エラーが発生した場合、データをリセット
    }
  };
    ...
    <div className="result-container p-6 max-w-lg mx-auto bg-white rounded-xl shadow-md space-y-4">
      ...
      {data && (
        <div className="mt-6 p-4 bg-gray-50 rounded-lg shadow-md">
          <h2 className="text-lg font-medium text-gray-900">取得した統計情報:</h2>
          <p className="mt-2 text-gray-700">PV: {data.page_view}</p>
          <p className="mt-2 text-gray-700">ユニークユーザー: {data.unique_user}</p>
          <p className="mt-2 text-gray-700">平均滞在時間: {data.like}</p>
        </div>
      )}
    </div>
  );
